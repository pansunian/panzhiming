...
const getNotionCalloutBg = (color?: string) => {
    ...
    return backgrounds[color || ''] || 'bg-[#f3efe6]';
};
        ...
        <div className="mt-12 border-t border-dashed border-stone-300/60 pt-7">
            ...
            <div className="flex flex-col gap-3">
                {posts.map((post, index) => {
                    ...
                    return (
                        <Link key={post.id} to={`/blog/${post.id}`} className="group flex min-h-[86px] overflow-hidden border border-stone-200/80 bg-paper-dark transition-colors hover:bg-[#eadfce]">
...
const NotionBlock: React.FC<{ block: any, isGallery: boolean }> = ({ block, isGallery }) => {
    ...
    switch (block.type) {
        ...
        case 'code':
            return (
                <div className="my-8 overflow-hidden rounded-lg border border-stone-200 bg-[#f3efe6] first:mt-0">
