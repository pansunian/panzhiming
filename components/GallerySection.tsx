        ...
        >
            <div className="flex flex-col h-full">
                <div className="relative w-full aspect-video overflow-hidden rounded-t-sm bg-stone-200">
                    ...
                    <div className="absolute top-4 left-4 bg-paper/90 text-ink text-[10px] font-mono px-2 py-1 shadow-sm">No. {group.ticketNumber}</div>
                </div>
                <div className="bg-paper p-5 relative border-t-2 border-dashed border-stone-200 rounded-b-sm flex-grow flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h3 className="font-serif font-bold text-xl leading-tight mb-1 group-hover:text-brand-accent transition-colors">{group.title}</h3>
...
export const GallerySection: React.FC<Props> = ({ groups, onViewAll, title = "影像辑" }) => {
  return (
    <section className="mb-16 scroll-mt-24 w-full">
      <div className="flex items-end justify-between mb-6 px-2">
         ...
         {onViewAll && (
             <Link to="/gallery" className="inline-flex items-center gap-1 font-mono text-[10px] text-stone-400 hover:text-ink transition-colors pb-1">
