...
线上默认读取构建时生成的静态 JSON 和静态图片，不再让访客实时等待 Notion。修改 Notion 后，推荐在 Vercel 里对当前 Production 部署执行一次 Redeploy，构建脚本会重新同步 Notion 内容。

如果临时想绕过静态内容，仍然可以访问 API 强制读取最新首页数据：
